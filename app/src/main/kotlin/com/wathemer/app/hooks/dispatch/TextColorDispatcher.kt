...
object TextColorDispatcher {
    ...
    /** Ledger names, built once at registration: setTextColor is a per-frame path on a scrolling list. */
    ...
    fun mapColor(id: Int, color: Int) {
        ...
        idColors[id] = color
        val name = labels.getOrPut(id) { "text/${WaIds.nameOf(id)}" }
        HookLog.arm(name, "#%08x".format(color))
        ensureHooked()
    }

    /** Index-aligned with [handlers], so the hot path names one without searching for it. */
    private val handlerLabels = ArrayList<String>()

    /** Register a fallback handler returning 0 to abstain. First non-zero wins. */
    fun addHandler(handler: (TextView) -> Int) {
        handlers.add(handler)
        handlerLabels.add("text/handler#${handlers.size}")
        HookLog.arm(handlerLabels.last())
        ensureHooked()
    }
